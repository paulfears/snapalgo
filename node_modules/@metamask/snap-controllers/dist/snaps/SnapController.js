...
exports.SNAP_PREFIX_REGEX = new RegExp(`^${utils_2.SNAP_PREFIX}`, 'u');
...
class SnapController extends controllers_1.BaseControllerV2 {
    constructor({ closeAllConnections, executeSnap, getRpcMessageHandler, messenger, state, terminateAllSnaps, terminateSnap, endowmentPermissionNames = [], npmRegistryUrl, idleTimeCheckInterval = 5000, maxIdleTime = 30000, maxRequestTime = 60000, fetchFunction = globalThis.fetch.bind(globalThis), featureFlags = {}, }) {
        super({
            messenger,
            metadata: {
                snapErrors: {
    ...
    async removeSnaps(snapIds) {
        ...
        await Promise.all(snapIds.map(async (snapId) => {
            ...
            const permissionName = (0, utils_2.getSnapPermissionName)(snapId);
    ...
    getPermittedSnaps(origin) {
        ...
        return Object.values((_a = this.messagingSystem.call('PermissionController:getPermissions', origin)) !== null && _a !== void 0 ? _a : {}).reduce((permittedSnaps, perm) => {
            if (perm.parentCapability.startsWith(utils_2.SNAP_PREFIX)) {
    ...
    async installSnaps(origin, requestedSnaps) {
        ...
        await Promise.all(Object.entries(requestedSnaps).map(async ([snapId, { version: rawVersion }]) => {
            const version = (0, utils_2.resolveVersion)(rawVersion);
            const permissionName = (0, utils_2.getSnapPermissionName)(snapId);
            if (!(0, utils_2.isValidSnapVersionRange)(version)) {
                result[snapId] = {
    ...
    async processRequestedSnap(origin, _snapId, versionRange) {
        ...
        if (existingSnap && (0, utils_2.getSnapPrefix)(snapId) !== utils_2.SnapIdPrefixes.local) {
    ...
    async updateSnap(origin, snapId, newVersionRange = utils_2.DEFAULT_REQUESTED_SNAP_VERSION) {
        ...
        if (!(0, utils_2.isValidSnapVersionRange)(newVersionRange)) {
    ...
    async _startSnap(snapData) {
        ...
        try {
            const result = await this._executeWithTimeout(this._executeSnap(Object.assign(Object.assign({}, snapData), { endowments: await this._getEndowments(snapId) })));
    ...
    async _set(args) {
        ...
        if ('manifest' in args) {
            ...
            (0, json_schemas_1.validateSnapJsonFile)(utils_2.NpmSnapFileNames.Manifest, manifest);
        }
        else {
        ...
        const snap = Object.assign(Object.assign({}, existingSnap), { enabled: true, id: snapId, initialPermissions,
            manifest, permissionName: (0, utils_2.getSnapPermissionName)(snapId), // so we can easily correlate them
    ...
    async _fetchSnap(snapId, versionRange = utils_2.DEFAULT_REQUESTED_SNAP_VERSION) {
        try {
            const snapPrefix = (0, utils_2.getSnapPrefix)(snapId);
            switch (snapPrefix) {
    ...
    async _fetchNpmSnap(packageName, versionRange) {
        if (!(0, utils_2.isValidSnapVersionRange)(versionRange)) {
            throw new Error(`Received invalid Snap version range: "${versionRange}".`);
        }
        const { manifest, sourceCode, svgIcon } = await (0, utils_2.fetchNpmSnap)(packageName, versionRange, this._npmRegistryUrl, this._fetchFunction);
    ...
    async _fetchLocalSnap(localhostUrl) {
        ...
        (0, json_schemas_1.validateSnapJsonFile)(utils_2.NpmSnapFileNames.Manifest, _manifest);
        ...
        (0, utils_2.validateSnapShasum)(manifest, sourceCode);
        return { manifest, sourceCode, svgIcon };
    }
    /**
    ...
    async getRpcMessageHandler(snapId) {
        ...
        const rpcHandler = async (origin, request) => {
            ...
            try {
                const result = await this._executeWithTimeout(handler(origin, _request));
     ...
     */
    async _executeWithTimeout(promise) {
    ...
    async calculatePermissionsChange(snapId, desiredPermissionsSet) {
        ...
        const newPermissions = (0, utils_1.setDiff)(desiredPermissionsSet, oldPermissions);
        // TODO(ritave): The assumption that these are unused only holds so long as we do not
        //               permit dynamic permission requests.
        const unusedPermissions = (0, utils_1.setDiff)(oldPermissions, desiredPermissionsSet);
        // It's a Set Intersection of oldPermissions and desiredPermissionsSet
        // oldPermissions ∖ (oldPermissions ∖ desiredPermissionsSet) ⟺ oldPermissions ∩ desiredPermissionsSet
        const approvedPermissions = (0, utils_1.setDiff)(oldPermissions, unusedPermissions);
