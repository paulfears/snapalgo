...
async function postBundle(options, code) {
    if (options.eval) {
        ...
        await (0, snaps_utils_1.evalBundle)(TEMP_BUNDLE_PATH)
            .catch((error) => {
            throw new Error(`Snap evaluation error: ${error.toString()}`);
        })
            .finally(async () => fs_1.promises.unlink(TEMP_BUNDLE_PATH));
    }
    if (options.manifestPath) {
        const { errors, warnings } = await (0, snaps_utils_1.checkManifest)(path_1.default.dirname(options.manifestPath), options.writeManifest, code);
...
class SnapsBrowserifyTransform extends stream_1.Transform {
    ...
    _flush(callback) {
        ...
        const result = (0, snaps_utils_1.postProcessBundle)(code, Object.assign(Object.assign({}, __classPrivateFieldGet(this, _SnapsBrowserifyTransform_options, "f")), { sourceMap: Boolean(inputSourceMap) && 'inline', inputSourceMap }));
        if (result.warnings.length > 0) {
            console.log(`Bundle Warning: Processing of the Snap bundle completed with warnings.\n${result.warnings.join('\n')}`);
        }
