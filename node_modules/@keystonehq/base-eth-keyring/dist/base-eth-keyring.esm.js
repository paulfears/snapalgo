...
class BaseKeyring {
  ...
  syncKeyring(data) {
    if (data.getRegistryType().getType() === extend.RegistryTypes.CRYPTO_HDKEY.getType()) {
      this.keyringMode = KEYRING_MODE.hd;
  ...
  async signPersonalMessage(withAccount, messageHex) {
    ...
    const requestId = v4();
    const ethSignRequest = EthSignRequest.constructETHRequest(Buffer.from(usignedHex, "hex"), DataType.personalMessage, hdPath, this.xfp, requestId, undefined, withAccount);
    const {
      r,
      s,
      v
  ...
  async signTypedData(withAccount, typedData) {
    const hdPath = await this._pathFromAddress(withAccount);
    const requestId = v4();
    const ethSignRequest = EthSignRequest.constructETHRequest(Buffer.from(JSON.stringify(typedData), "utf-8"), DataType.typedData, hdPath, this.xfp, requestId, undefined, withAccount);
    const {
      r,
      s,
      v
...
export { BaseKeyring };
//# sourceMappingURL=base-eth-keyring.esm.js.map
